//服务器地址
_serverAddress = _server + "/cop_et/etArcrpExg/list";
//跳转页面
    ...
    $("#gdecd").keydown(function (event) {
        if (optype !== "view") {
            if (event.keyCode === 13) {
                ...
                var modelParam = {};
                modelParam.area = ["800px", "450px"];
                modelParam.url = _jumpPage + "/complexChooseList.jsp?gdecd=" + gdecd + "&seqNo=" + seqNo + "&gridId=exgTable";
...
function __onAfterLoadCodes() {
    ...
    $("#uid").val(id);
    if (optype === "view") {
        _serverAddress = _server + "/cop_et/et" + view + "Exg/list";
    }
    FormUtils.getData();
}
