...
var columns = [
    ...
    {title: "料件序号", field: "gdsSeqno", align: "center", sortable: true, order: "asc"},
    {title: "料件料号", field: "gdsMtno", align: "center", sortable: true},
...
$(function () {
    ...
    //导入中间表数据
    $("#getMidImg").click(function () {
         $.ajax({
            url: _serverAddress+"/getMidImgData",
            data: {},
            dataType: 'json',
            xhrFields: {
    ...
    $("#genEmsPurImg").click(function(){
        showPage('purImg','企业原始料件选择','../erpPreDtImg/select.jsp');
    });
...
function initDropDown() {
    var data=[{id:"1",text:"全部"},{id:"2",text:"已备案"},{id:"3",text:"未备案"},{id:"4",text:"保税"},{id:"5",text:"非保税"}];
...
function view(optype) {
    if (optype == "add") {
        //Utils.redirect(_jumpPage + "edit.jsp");
...
function modify(optype) {
    ...
    if (successFlag=="1" && optype=="edit") {
        layer.msg("料件已生成备案，不允许修改", {time: 1500});
        return;
    }
    ...
    //Utils.redirect(_jumpPage + "edit.jsp?id=" + id + "&optype=" + optype);
...
function removeData() {
    ...
    var successFlag  = rows[0]["successFlag"];
    if (successFlag=="1") {
        layer.msg("料件已生成备案，不允许删除", {time: 1500});
        return;
    }
    var modfMarkcd = rows[0]["modfMarkcd"];
