...
function SetDefault() {
    ...
    SetValue("mtpckEndprdTypecd", "I");    // 料件成品标记代码 I-料件
    // 申报序号 获取
    $.ajax({
    ...
    switch(busType){
        case "jg":
            var mtpckEndprdTypecd = Utils.search("mtpckEndprdTypecd");
            if (mtpckEndprdTypecd == "I"){
                url=_server + "/ems/emsCusImg/list/selectMaxGdsSeqno?emsNo=";//料件
            }else {
                url=_server + "/ems/emsCusExg/list/selectMaxGdsSeqno?emsNo=";//成品
            }
            break;
        case "wl":
        case "jdjg":
        case "wf":
        case "ls":
            url=_server + "/ems_bws/emsBwsCusDt/list/selectMaxGdsSeqno?bwsNo=";//物流
            break;
        default : layer.msg("数据错误，不可获取");
    }
    $.ajax({
