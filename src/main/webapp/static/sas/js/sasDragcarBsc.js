//服务器地址
_serverAddress = _server + "/sas/sasDragcarBsc/list";
//跳转页面
_jumpPage = baselocation + "/views/sas/sasDragcarBsc/";
...
if(viewType=="cusTable"){
    ...
    _serverAddress = _server + "/sas/sasDragcarHis/list";
}
...
var serverUrl = {busTab: "/sas/sasDragcarBsc/list", cusTab: "/sas/sasDragcarCus/list", hisTab: "/sas/sasDragcarHis/list"};
var tab = "bscTable";
//页面绑定事件
$(function () {
    if (id == null) {
    ...
    if (optype=="add"){
        $("#table-head").html("托卡备案-新增")
    } else if(optype=="edit" ){
        $("#table-head").html("托卡备案-修改")
    }else if(optype=="view" ){
        $("#table-head").html("托卡备案-查阅")
    }else if(optype=="change" ){
        $("#table-head").html("托卡备案-变更")
    }
...
function SetDefault() {
   ...
   /* if ($("#seqNo").val() == "") {
        //获取企业预录入编号
        Utils.getBillCode('applyId=001&areaCode=490101&docType=A0504CopNO&serverType=C', 'etpsPreentNo', '企业预录入编号获取失败');
        //获取申报端统一编号
        Utils.getBillCode('applyId=001&areaCode=490101&docType=A0504SeqNO&serverType=C', 'seqNo,dclPreentNo', '申报端统一编号获取失败');
    }*/
}
...
function getdata(data) {
    layer.closeAll();
    Utils.showEditDiv(encodeURI(_jumpPage+"add.jsp?optype=change&data="+encodeURI(JSON.stringify(data))));
}
