...
*/
$(function(){
    $('#kaptchaImage').attr('src', _server+'/captchaMain-image.jpg?' + Math.floor(Math.random()*100) ).click(function () {
        $(this).attr('src', _server+'/captchaMain-image.jpg?' + Math.floor(Math.random()*100) );
    })
});
		...
		success:function(result){
			...
			if(result.code!=1){
				if(result.message=="验证码错误"){
					...
					$("input[name='systemUser.loginPassword']").attr("placeholder",result.message);
				}else{
					layer.alert(result.message, {icon: 2});
				}
			}else{
				window.location.href=baselocation+'/views/admin/main/main.jsp';
			}
		}
	});
	})
